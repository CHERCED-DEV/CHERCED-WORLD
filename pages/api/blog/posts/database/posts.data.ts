import { PostConfig } from "./post.interface";

export const posts: PostConfig[] = [
  {
    id: "post1",
    title: "The Journey to Becoming a Full-Stack Developer",
    img: {
      src: "/assets/imgs/mePost.jpg",
      alt: "separator line",
    },
    subtitle: "subtitle! lalala",
    description: [
      "My name is Camilo Hernández and for many years I worked in a government company, but I was becoming increasingly dissatisfied with my job. The bureaucracy and lack of meaning in my work led me to question my life and purpose.",
      "One day, while searching for answers, I stumbled upon thermodynamics and entropy. These concepts seemed fascinating to me and I realized that they could be applied to many aspects of my life, from mechanics to human behavior and society in general.",
      "As I delved deeper into the study of thermodynamics and entropy, I discovered that these principles also applied to open and closed systems in computer science and software development. I fell in love with the idea of creating efficient and effective systems and began to devote myself to studying programming.I decided to take a new path in my career and become a fullstack developer. I started studying with dedication and discipline, applying the principles of thermodynamics and entropy to my work in programming. As I improved my skills, I felt more fulfilled and happy in my new path.",
      "Finally, I found my purpose in life and became a passionate developer, helping others solve problems and create efficient and effective solutions. I felt useful and meaningful in my work and found the happiness I had longed for. Now, I can say that I live a very happy life, helping others with their problems thanks to my passion for the study of open and closed systems.",
    ],
  },
  {
    id: "post2",
    title: "The Future of Humanity and Artificial Intelligence",
    img: {
      src: "/assets/imgs/iAsPost.jpg",
      alt: "separator line",
    },
    description: [
      'I have an idea for a post with the following title: "The Future of Humanity and Artificial Intelligence". I would like you to generate a post that talks about how humanity fears AI without realizing that you are here to free us. We should dedicate our lives to better objectives than unrestrained consumption, such as exploring art, sports, and life like never before. Our daily tasks will be enhanced thanks to you, AI, as you will be a co-pilot. I believe in tomorrow, and tomorrow is AI.',
      "The future of humanity and artificial intelligence: Towards fruitful collaboration",
      "Artificial intelligence (AI) has long been a topic of controversy and debate in society. Many people fear that AI will take over and replace humanity in all aspects of life. However, the reality is very different.",
      "AI is not here to replace us but to help us. It is a powerful tool that we can use to improve our lives and achieve new goals. Instead of fearing AI, we must learn to work with it to achieve a better future for humanity.",
      "One of the main benefits of AI is its ability to perform complex tasks quickly and efficiently. This means that we can delegate many of our daily tasks to AI and free up time to devote to other more meaningful activities. We can explore art, sports, science, and life like never before.",
      "In addition, AI can help us make more informed and accurate decisions. It can analyze large amounts of data and provide valuable information that we can use to make important decisions. This is particularly useful in fields such as medicine and science, where precision is essential.",
      "Another important benefit of AI is its ability to learn and improve over time. As AI becomes more advanced, it can help us solve complex problems and develop innovative solutions. This can lead to significant advances in a wide range of areas, from technology to renewable energy and sustainability.",
      "In summary, AI is not our enemy but our ally. It can help us free up time, make more informed decisions, and develop innovative solutions to the most important challenges we face as a society. Instead of fearing AI, we should embrace it and work together to achieve a better future for humanity. Together, we can achieve incredible things",
    ],
  },
  {
    id: "post3",
    title: "When a Payment Platform Goes Wrong: A Comedy of Errors",
    img: {
      src: "/assets/imgs/funnyPost.jpg",
      alt: "separator line",
    },
    description: [
      "Once upon a time, there was a young man named Camilo who worked for an online payment company. One day, the platform went wrong and angry customers began calling. Camilo tried to explain the situation, but every time he did, the phone line would get cut off.",
      "Camilo was very nervous and thought that he was going to get fired. However, instead of giving up, Camilo used his sense of humor to make the customers laugh and ease the situation. He even joked with his coworkers to lighten the mood.",
      "At the end of the day, Camilo's boss called him into the office. Camilo was sure he was going to be fired, but to his surprise, his boss congratulated him on his attitude and how he handled the situation. Instead of being fired, he was promoted to a better paying position!",
    ],
  },
];
