...
export async function getCMSData(): Promise<CmsDataConfig> {
    const response = await fetch(`https://${process.env.NEXT_PUBLIC_VERCEL_URL}/api/customCMS`);
    return response.json();
} 

export async function fetchProject(id: any): Promise<ProjectsConfig> {
    const res = await fetch(`https://${process.env.NEXT_PUBLIC_VERCEL_URL}/api/projects/${id}`);
    return res.json();
}
